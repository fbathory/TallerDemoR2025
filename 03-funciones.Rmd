...
Primero vamos a crear un data frame simple con datos ficticios
de Argentina:

```{r datos_mig, message=F, warning=F}
...
Creamos la función con sus validaciones:

```{r tasa, message=F, warning=F}
...
```{r funcion_mig, message=F, warning=F}
...
```{r TEF_data, warning=F, message=F}
...
Si lo queremos graficar:

```{r graf_TEF, warning=F, message=F}
...
```{r}
tgf <- tasa_global_fecundidad(nac, muj)
tgf
...
paste("TGF estimada:", round(tgf, 2))
