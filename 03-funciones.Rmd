...
Una función es un bloque de código reutilizable, diseñado para tomar uno o más argumentos y devolver un resultado. Permite **encapsular**  un procedimiento , que recibe argumentos y devuelve un resultado, para no repetir código y evitar errores. 
...
# miremos un gráfico
data.frame(Año = c(2020,  2030,  2035),
...
Obtengamos la población y defunciones de *Chile*, *Japón* y *Polonia* del año 2000 y 2010 leyendo la tabla "Chile_Japon_Polonia.csv" en la carpeta "data". 
...
standarizarrr(readxl::read_xlsx("data/Chile_Japon_Polonia.xlsx") %>% filter(Year==2020))
```

b) Con los datos de la tabla "Chile_Japon_Polonia.csv": ¿qué se puede decir sobre la evolución de la mortalidad de Polonia en los años 1990, 2000 y 2010? ¿Bajó o creció?
...
jpn_2020 <- db %>% filter(Country == "Japon", Year == 2020)

# obtengo la tabla
library(DemoTools)
tm_jpn_2020 <- lt_abridged(nMx = jpn_2020$M, Age = jpn_2020$Age)
