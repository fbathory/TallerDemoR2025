...
```{r, eval=F}
eder_mayores <- eder_mayores %>% arrange(p3b)
eder_mayores
# mejor al revés: descendente
eder_mayores <- eder_mayores %>% arrange(desc(p3b))
```
...
Todo gráfico esta compuesto, jerárquicamente, por:
...
```{r primer plot, message=F, warning=FALSE}
...
```{r eder por sexo 1, message=F, warning=FALSE}
...
```{r eder por sexo, message=F, warning=FALSE}
...
```{r eder por sexo-colores, warning=FALSE}
...
```{r nivel ed1, message=F, warning=FALSE}
...
```{r nivel ed2, message=F, warning=FALSE}
