...
En la unidad previa caracterizamos **un** hogar censal. En esta unidad trabajaremos la **base** de datos de la [Encuesta Demográfica Restropectiva (EDER)](https://www.estadisticaciudad.gob.ar/eyc/?page_id=118675) que se realizó en CABA en el año 2019. Esta incluye cuatro bases: 
...
Seguro ya estas cansado de levantar datos: "Dame la EDER" `r fa("meh")` se escucha murmurar en el aula-zoom.
...
sum(DEf05_22$cantidad[DEf05_22$anio == 2021])
sum(DEf05_22$cantidad[DEf05_22$anio == 2021 & DEf05_22$sexo_id == 1])
sum(DEf05_22$cantidad[DEf05_22$anio == 2021 & DEf05_22$jurisdiccion_de_residencia_id == 82])
```
...
a) Leer *"eder2019_usuarios_personas.csv"* (por deafult con separador ",") y guardarlo en un objeto llamado _eder_personas_br_ (*br* por bruta, aún sin cambios).  

```{r, include=F, hide=TRUE}
eder_personas_br <- read.csv("data/EDER_CABA/eder2019_personas.csv", header = T, sep = ",")
```

b) Explorar el objeto leído en el punto anterior con lo visto hasta quí, y resaltar aspectos que te llamen la atención.  
...
Previo a comenzar debes haber leído la base de la EDER *"eder2019_usuarios.personas.txt"*, haberla inspeccionado brevemente y guardado en un objeto llamado _eder_personas_br_ (ver \@ref{intro}). 
...
a) Crear un *data.frame* llamado "eder_personas_edu" que contenga solo las variables que refieran a situación educativa (que tienen un "e" al inicio) de "eder_personas_br". Puedes utilizar la función `starts_with` (leer [ejemplos](https://tidyselect.r-lib.org/reference/starts_with.html)).
...
a) Renombrar las variables de manera que siempre comiencen con "EDU" (en vez de *e2* que sea *ID_VIVIENDA*).
...
*BONUS*: Si queremos considerar el total de personas expandido, podemos utilizar la función `sum(factor expansión)`:
